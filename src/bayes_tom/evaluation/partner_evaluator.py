...
def generate_offline_dataset(config, env, rng, num_episodes, ego_agent, ego_params,
                            partner_population, partner_params, save_path, 
                            specific_partner_idx=None):
    '''Generate offline RL dataset from rollouts with both ego and partner agent data
    
    Args:
        config: Configuration dictionary
        env: Environment instance
        rng: JAX random key
        num_episodes: Number of episodes to collect per partner
        ego_agent: Ego agent instance
        ego_params: Parameters for ego agent
        partner_population: Partner population instance
        partner_params: Parameters for partner population
        save_path: Path to save the HDF5 dataset file
        specific_partner_idx: If specified, only generate data for this partner index.
                             If None, generate data for all partners (default).
    '''
    num_agents = env.num_agents
    assert num_agents == 2, "This dataset generation code assumes exactly 2 agents."

    num_partner_total = partner_population.pop_size
    partner_child_idx = config.get("task", {}).get("agent_model", {}).get("partner_child_idx", 0)
    
    # Determine which partners to generate data for
    if specific_partner_idx is not None:
        assert 0 <= specific_partner_idx < num_partner_total, \
            f"specific_partner_idx {specific_partner_idx} out of range [0, {num_partner_total})"
        partner_indices = [specific_partner_idx]
        print(f"Generating dataset for partner {specific_partner_idx} only")
    else:
        partner_indices = list(range(num_partner_total))
        print(f"Generating dataset for all {num_partner_total} partners")
    ...
    pbar = tqdm(
        desc="Generating offline dataset...",
        total=num_episodes * len(partner_indices)
    )
    ...
    for partner_idx in partner_indices:
        ...
        for episode_idx in range(num_episodes):
            ...
            # Pre-allocate lists for JAX arrays (avoid immediate conversion)
            episode_ego_obs = []
            episode_ego_next_obs = []
            episode_ego_actions = []
            episode_ego_rewards = []
            episode_ego_dones = []
            episode_ego_avail = []
            ...
            episode_partner_obs = []
            episode_partner_next_obs = []
            episode_partner_actions = []
            episode_partner_rewards = []
            episode_partner_dones = []
            episode_partner_avail = []
            
            # Reset the env
            partner_rng, reset_rng = jax.random.split(partner_rng)
            obs, env_state = env.reset(reset_rng)
            done = {k: jnp.zeros((1), dtype=bool) for k in env.agents + ["__all__"]}
            act_onehot = {k: jnp.zeros((env.action_space(env.agents[i]).n)) for i, k in enumerate(env.agents)}
            reward = {k: jnp.zeros((1)) for i, k in enumerate(env.agents)}
            ...
            # No trace needed for dataset generation - saves memory and computation
            trace = None

            # Initialize hidden states
            hstate_0 = ego_agent.init_hstate(1, aux_info={"agent_id": 0})
            hstate_1 = _init_partner_hstate(partner_population, partner_idx, partner_child_idx)
            partner_child_idx_batched = jnp.array([partner_child_idx])
            ...
            while t <= max_episode_steps:
                ...
                partner_rng, act0_rng, act1_rng, step_rng = jax.random.split(partner_rng, 4)

                # Get ego action
                act_0, hstate_0 = ego_agent.get_action(
                    params=ego_params,
                    partner_indices=partner_idx_batched,
                    obs=obs["agent_0"].reshape(1, 1, -1),
                    done=done["agent_0"].reshape(1, 1),
                    avail_actions=avail_actions_0,
                    hstate=hstate_0,
                ...
                act_1, hstate_1 = _partner_action(
                    partner_population=partner_population,
                    partner_params=partner_params,
                    partner_idx_batched=partner_idx_batched,
                    child_idx_batched=partner_child_idx_batched,
                    obs=obs["agent_1"].reshape(1, 1, -1),
                    done=done["agent_1"].reshape(1, 1),
                    avail_actions=avail_actions_1,
                ...
                # Store JAX arrays directly (no conversion yet - much faster!)
                episode_ego_obs.append(obs["agent_0"])
                episode_ego_actions.append(act_0)
                episode_ego_rewards.append(reward["agent_0"])
                episode_ego_next_obs.append(next_obs["agent_0"])
                episode_ego_dones.append(done["agent_0"])
                episode_ego_avail.append(avail_actions_0.squeeze())
                
                episode_partner_obs.append(obs["agent_1"])
                episode_partner_actions.append(act_1)
                episode_partner_rewards.append(reward["agent_1"])
                episode_partner_next_obs.append(next_obs["agent_1"])
                episode_partner_dones.append(done["agent_1"])
                episode_partner_avail.append(avail_actions_1.squeeze())

                # No trace needed for dataset generation
            ...
            # Convert entire episode at once (single device sync instead of hundreds)
            episode_len = len(episode_ego_obs)
            ego_transitions['observations'].extend(np.array(jnp.stack(episode_ego_obs)))
            ego_transitions['actions'].extend(np.array(jnp.stack(episode_ego_actions)))
            ego_transitions['rewards'].extend(np.array(jnp.stack(episode_ego_rewards)))
            ego_transitions['next_observations'].extend(np.array(jnp.stack(episode_ego_next_obs)))
            ego_transitions['dones'].extend(np.array(jnp.stack(episode_ego_dones)))
            ego_transitions['avail_actions'].extend(np.array(jnp.stack(episode_ego_avail)))
            ego_transitions['partner_idx'].extend([partner_idx] * episode_len)
            ego_transitions['episode_idx'].extend([episode_idx] * episode_len)
            
            partner_transitions['observations'].extend(np.array(jnp.stack(episode_partner_obs)))
            partner_transitions['actions'].extend(np.array(jnp.stack(episode_partner_actions)))
            partner_transitions['rewards'].extend(np.array(jnp.stack(episode_partner_rewards)))
            partner_transitions['next_observations'].extend(np.array(jnp.stack(episode_partner_next_obs)))
            partner_transitions['dones'].extend(np.array(jnp.stack(episode_partner_dones)))
            partner_transitions['avail_actions'].extend(np.array(jnp.stack(episode_partner_avail)))
            partner_transitions['partner_idx'].extend([partner_idx] * episode_len)
            partner_transitions['episode_idx'].extend([episode_idx] * episode_len)
    ...
    for key in ego_transitions:
        ego_transitions[key] = np.array(ego_transitions[key])
    
    for key in partner_transitions:
        partner_transitions[key] = np.array(partner_transitions[key])
    ...
    with h5py.File(save_path, 'w') as f:
        # Create ego agent group
        ego_group = f.create_group('ego_agent')
        for key, value in ego_transitions.items():
            ego_group.create_dataset(key, data=value, compression='gzip')
        
        # Create partner agent group
        partner_group = f.create_group('partner_agent')
        for key, value in partner_transitions.items():
            partner_group.create_dataset(key, data=value, compression='gzip')
        
        # Save metadata
        f.attrs['num_episodes'] = num_episodes
        f.attrs['num_partners'] = len(partner_indices)
        f.attrs['total_transitions'] = len(ego_transitions['observations'])
        f.attrs['env_name'] = config["ENV_NAME"]
        if specific_partner_idx is not None:
            f.attrs['specific_partner_idx'] = specific_partner_idx
        
    print(f"Dataset saved successfully!")
    print(f"Total transitions collected: {len(ego_transitions['observations'])}")
    print(f"Ego agent transitions shape: {ego_transitions['observations'].shape}")
    print(f"Partner agent transitions shape: {partner_transitions['observations'].shape}")
    
    return ego_transitions, partner_transitions
...
def run_partner_evaluation(config, print_metrics=False):
    ...
    if mode == 'probe':
        ...
        specific_partner_idx = config.get("task", {}).get("agent_model", {}).get("dataset_partner_idx", None)
        
        # Handle dataset path - if None or a directory, construct a proper file path
        if dataset_path is None or os.path.isdir(dataset_path):
            base_dir = dataset_path if dataset_path is not None else config["task"]["results_path"]
            if specific_partner_idx is not None:
                dataset_path = os.path.join(base_dir, f"offline_dataset_partner_{specific_partner_idx}.hdf5")
            else:
                dataset_path = os.path.join(base_dir, "offline_dataset.hdf5")
        ...
        ego_transitions, partner_transitions = generate_offline_dataset(
            config, env, eval_rng, dataset_episodes, 
            ego_agent, flattened_ego_params, 
            train_partner_population, train_flattened_partner_params,
            save_path=dataset_path,
            specific_partner_idx=specific_partner_idx
        )
        return ego_transitions, partner_transitions
