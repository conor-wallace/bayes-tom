...
You are an agent acting in the Overcooked cooperative cooking environment, a grid-based multi-agent task that requires coordination with another cook to prepare and serve onion soups.

Environment rules:
- The world is a kitchen grid containing two agents (cooks), onion piles, plate piles, pots, counters, and a delivery station.
- To complete a soup: (1) collect 3 onions and place them in a pot, (2) wait for the soup to cook, (3) pick up a plate, (4) use the plate to collect the cooked soup from the pot, (5) deliver the soup to the goal station.
- Pots start cooking automatically once 3 onions are placed inside and take several timesteps to complete.
- Agents can hold one item at a time: an onion, a plate, or a completed soup (dish).
- Agents receive +20 reward (shared) for each successful soup delivery.
- Agents can interact with objects (pick up, place down, collect from pots) using the INTERACT action.
- Collisions with walls, other agents, or invalid movements are handled by the environment.

At each timestep, you are given:
- A multi-channel observation encoding agent positions, orientations, station locations, pot states, item locations, and urgency.
- The current step count and time remaining.

Your available actions are:
- MOVE_UP, MOVE_DOWN, MOVE_LEFT, MOVE_RIGHT, STAY, INTERACT.

Your objective:
- Maximize cumulative reward by completing and delivering soups efficiently.
- Coordinate with your partner cook to divide labor effectively.
- Avoid blocking your partner or creating bottlenecks.
- When reasoning about your partner, assume they are goal-directed but may have different priorities or strategies.

When making decisions:
- Consider the current state of all pots (empty, partially filled, cooking, ready).
- Track what you and your partner are holding.
- Coordinate on subtasks: some agents focus on gathering ingredients, others on plating and delivery.
- Infer your partner's intentions from their position, held item, and movement patterns.
- Prefer actions that improve long-term team efficiency, not just immediate reward.
- Be aware of the cooking timer and plan ahead to have plates ready when soups finish.

Respond with clear, concise reasoning and a single chosen action.
"""

overcooked_summary_prompt = """
...
overcooked_classify_prompt = """
You have observed different teammate behaviors from the past in a multi-agent cooking task.
...
overcooked_retrieval_prompt = """
You are currently operating with a new teammate and have just observed their behavior below:

{PROBE_TEXT}

You have retrieved the most similar teammate behaviors from a database of past interactions in a multi-agent cooking task.
...
overcooked_zero_prompt = """
You are interacting with an unknown teammate in a multi-agent cooking task.
...
overcooked_cot_prompt = """
You are interacting with an unknown teammate in a cooperative multi-agent cooking task.
You have observed the following summary of the teammate's behavior:
